...
TBD - defines editor IntelliSense in the VSCode extension for authoring `<<<` snippet imports: path completion under the configured code root, and commands to copy a selector for the current selection and paste a selector into an existing import line.
