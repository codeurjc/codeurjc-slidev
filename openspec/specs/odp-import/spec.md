...
Converts a LibreOffice Impress (ODP) deck into a Slidev `slides.md` on a best-effort basis: one slide per ODP slide, cover/copyright layouts, titles and in-content headings written with minimal repetition through title carry-over, lists, inline formatting, links, tables, images positioned with slide geometry, build-ups merged into click steps when fully convertible, and every loss reported to the console instead of into the slides.
