...
How slide frontmatter names one of the slide's images (`geometry.images` entries, callout image anchors): by the `src` the author wrote, with `#N` for a picture shown more than once, or by the legacy position. References resolve against the authored `src` in dev and built decks, never re-target another image, and every writer produces the shortest unambiguous form.
