# slide-geometry-inspection

## Purpose

Lets an external controller -- the VS Code extension -- drive geometry inspection in whatever preview of a deck is open, over a channel the theme's own dev-server plugin owns: labelled outlines over the real render for every `geometry` entry (including entries that match nothing, which otherwise leave no visible trace), highlighting a single entry, and taking over geometry writes so drags are reported to the controller instead of written to frontmatter.
