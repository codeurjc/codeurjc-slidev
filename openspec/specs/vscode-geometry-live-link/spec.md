# vscode-geometry-live-link

## Purpose

Links the open deck to a running dev server's preview: the geometry entry under the cursor is highlighted in the preview, and elements dragged there are applied to the open document by their stable key and saved, reusing the official Slidev extension's preview rather than providing one.
