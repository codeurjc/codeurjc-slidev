...
import { findFitFontSize, AUTOFIT_MIN_PX, AUTOFIT_MAX_PX } from '../useAutoFitText'

describe('findFitFontSize', () => {
  it('returns the max size when content never overflows', () => {
    const overflows = vi.fn(() => false)
    const size = findFitFontSize(AUTOFIT_MIN_PX, AUTOFIT_MAX_PX, overflows)
    expect(size).toBe(AUTOFIT_MAX_PX)
  ...
  it('returns the min size when content overflows even at the floor', () => {
    ...
    const size = findFitFontSize(AUTOFIT_MIN_PX, AUTOFIT_MAX_PX, overflows)
    expect(size).toBe(AUTOFIT_MIN_PX)
  })
  ...
  it('converges on a size between min and max when overflow depends on size', () => {
    ...
    expect(size).toBeGreaterThan(AUTOFIT_MIN_PX)
  ...
  it('never calls overflows with a size outside [min, max]', () => {
    ...
    findFitFontSize(AUTOFIT_MIN_PX, AUTOFIT_MAX_PX, overflows)
    for (const size of seen) {
      expect(size).toBeGreaterThanOrEqual(AUTOFIT_MIN_PX)
      expect(size).toBeLessThanOrEqual(AUTOFIT_MAX_PX)
    }
  })
  ...
  it('is bounded by maxIterations', () => {
    ...
    findFitFontSize(AUTOFIT_MIN_PX, AUTOFIT_MAX_PX, overflows, 3)
