...
- `src/referenceIndex/` — the reverse-reference feature: `indexBuilder.ts` builds, per target file, a *recipe* of which slide/anchor-line/selector combinations reference it (no file reads, no absolute line numbers yet); `codeLens.ts` resolves those recipes against a target file's live editor text on demand, so an edit to the target file alone is reflected immediately without any index invalidation — only an edit to the markdown file itself requires rebuilding that file's contribution (`updateReferenceIndexForFile`). `scanner.ts` is the only fs-touching part (workspace enumeration + `@/...` path resolution against the code-root convention, plus `listCodeRootDirectory` for path completion below).
...
- `src/extension.ts` — thin adapter wiring the above to real `vscode` decorations/Hover/Diagnostic/CodeLens/CompletionItem providers and the two selector commands; deliberately not unit tested itself (see Tests below).
