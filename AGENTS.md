...
- `composables/useEditor.ts` — singleton state for element positions, undo, drag/resize, and save (the four fixed elements `red-bar`/`logo`/`title`/`content` + dynamic per-slide keys, e.g. callouts). Editor mode (`editing`) has two writers: the SideEditor's Layout tab and an external controller (`setControllerEditing`); it's on while either wants it, and every `editing.value = …` write goes to the tab's source
...
- `global-top.vue` + `composables/useImagePosition.ts` — image paste and the Below/Right position presets, which write the slide's `geometry` (see "Paste presets" below)
- `composables/slideInfoSync.ts` — a `window` event carrying a slide's info after a frontmatter write made outside the layout (paste presets). `global-top.vue` doesn't always share the `useDynamicSlideInfo` cache the layout and the side editor read, so each applies the published info to its own copy
...
- `composables/useInspectProtocol.ts` — the controller channel's message shapes (commands in, events out) and their validation, with no imports so `vite.config.ts` can load it; `composables/useInspectKeys.ts` — the stable key a geometry drag is reported under; `composables/useInspectClient.ts` — the page's end of the channel (see "Geometry inspection" below)
- `composables/stepRange.ts` — the click step grammar shared by markers, anchors, slide callouts, the ODP importer and the VS Code click model: `{N}`/`{N-}` (from click N on), `{N-M}` (clicks N through M inclusive), `{-M}` (from click 0 through M; `{-0}` is only before the first click). Parsing and validation, the shortest written form, visibility at a click, the clicks to register with Slidev (the start and the click after the end) and interval overlap for placement
- `composables/fenceInfo.ts` — a fence info string split like Slidev's own code block wrapper (`lang [title] {ranges} {options}`), used by the theme's wrapper and the VS Code click model
...
Pasting an image (`global-top.vue`) uploads it to `public/images/paste-<ts>.<ext>` and inserts `![](…)` into the slide. On a `default`-layout slide, a Below/Right popover then anchors to the pasted image, found by its authored `src` (`data-src`) and so found on slides that already have geometry, or several images.

- **A choice writes geometry.** It sets `geometry.content` and replaces or appends the image's `src`-keyed `geometry.images` entry (`withPositionedImage`), keeping every other entry. The write goes through `update({ frontmatter })`: there is no layout file, no `layout:` change and no reload. The one exception is slide 1: its frontmatter is the deck's headmatter, and Slidev reloads the page when that changes.
- **The popover stays open** until ✕ or a slide change, and re-anchors to the moved image.
- **Presets start from the effective content rect**: `geometry.content` when declared, else the layout's content position.
- **Right** narrows the content box and puts the image in the freed right-hand part.
- **Below** splits the height under the content's top: the content takes its rendered text height clamped to 30–60%, and the image is fitted into the rest (at most 80% of the canvas width, centred, 24 px gap). The image always stays on the canvas.
- **Side editor.** Slidev's response to a frontmatter patch still carries the old `frontmatterRaw`, which the textarea is built from. `global-top.vue` therefore waits for the server's reparse and publishes the fresh info (`slideInfoSync.ts`), so a later textarea save can't write the old frontmatter back.

### Geometry inspection

An external controller (the VS Code extension) can outline every `geometry` entry over the real render, highlight one, and take over geometry writes, in whatever preview is open. Protocol in `composables/useInspectProtocol.ts`; page side in `composables/useInspectClient.ts` (installed from `global-top.vue`) and `layouts/default.vue`; relay in `vite.config.ts` (`codeurjc-slidev-geometry-inspect`).

- **Why a dev-server channel.** An embedded preview's URL is built by whoever embeds it (the official Slidev extension writes `<server>/<n>?embedded=true` itself), and its relay only forwards Slidev's own `target: 'slidev'` messages. Slidev hides its "Show editor" button when embedded, and `showEditor`'s localStorage is partitioned away from browser tabs. So commands travel over Vite's HMR socket, which reaches the page whatever its URL.
- **Wire.** The controller reads a server-sent-events stream at `/api/geometry-inspect/events` and POSTs commands to `/api/geometry-inspect`; the server relays both ways as HMR custom events. It writes an SSE comment on attach (Node sends no headers until the first body write, so an attach with no page open would otherwise never complete).
- **Commands.** `inspect` (outlines; **implies editor mode**, so drag handles show, and turning it off restores the Layout tab's own state), `highlight` (one entry by slide, collection and index; an index the slide no longer has highlights nothing), `control` (claim geometry writes), `whichDeck`.
- **Deck identity comes from the page**, not the server: Vite's resolved `config.slidev` is the theme-facing plugin options, not Slidev's resolved entry. Each page answers `whichDeck` with slide 1's source file (the headmatter is always in the entry deck).
- **State survives reloads and departures.** The server remembers the controller's state and replays it when a page loads and asks (`INSPECT_SYNC_EVENT`). When the last controller detaches it releases control and inspection, so pages never keep emitting drags into the void.
- **Outlines.** Each entry's overlay is tagged `elements[0] · Element flow`. An entry that matched nothing gets `geometry-unresolved` and "· matches nothing": it has no other visible trace, since its element just stays in flow.
- **Controlled mode.** `persistGeometry` emits `{ slideNo, key, from, to }` per changed entry instead of patching frontmatter; the editor's live rect keeps the element where it was dropped. The key is stable (`content`, an `elements` key, or an image src; a positional `images` entry is named by its image's src, `inspectKeyFor`), never a list index.
- **Slide 1 caveat.** Writing slide 1's frontmatter rewrites the headmatter, and Slidev reloads the page. The state replay covers it; e2e fixtures that need a stable page put their geometry on slide 2.
...
- **Connector style** (`calloutStyle` in a slide's frontmatter, or deck-wide through the headmatter's `defaults: { calloutStyle: elbow }`), for slide callouts *and* code callouts. It is read from `$frontmatter`, the only frontmatter Slidev merges `defaults:` into; the editor never writes it.
  - `arrow` (the default): `arrowPath(anchorPoint(highlightRect, side), box)`, a straight segment from the highlight edge facing the box (or the anchor point) to where the line from the box's centre towards it leaves the box. The larger box-end head sits on `marker-end`, pointing into the box. Placement keeps boxes `ARROW_OBSTACLE_GAP` (36px) from the block or element instead of 12px, so there's room for the head, and a connector shorter than `MIN_ARROWHEAD_CONNECTOR` (30px, e.g. a box dragged next to its anchor) is drawn as a plain line.
  - `elbow`: `elbowPath` returns `[anchor, bend, boxEdge]`, with the small anchor-end head on `marker-start` (`orient="auto-start-reverse"`), pointing at the anchor.
  - A bare arrow keeps its short stub with the head on the anchor in both styles. The `<marker>`s carry their own fill because `.code-callout-connector` sets `fill: none`, and their ids are unique per layout instance (`useId()`): Slidev keeps neighbouring slides mounted but hidden, and `url(#id)` resolves to the first element with that id, so a shared id pointed connectors at a hidden slide's marker, which the browser doesn't render.
- **Click steps.** `step: N`, or a step range written as a YAML string (`step: 2-4`, `step: -1`; see `stepRange.ts`), counts toward the slide's clicks the same way code-highlight steps do: a `post` markdown transformer (`slideCalloutStepBlock` in `setup/transformers.ts`) appends a hidden block of `v-click="N"` placeholders, one per distinct click at which the slide's valid callouts change (each start, and the click after a range's end), to every `default`-layout slide's markdown. It has to be compiled into the slide, since the layout only reads callouts after mount, too late for Slidev to count them. An editor write that changes a step only reaches the placeholders once the slide's markdown is re-rendered.
...
- `<comment>` — everything after the closing `]` up to the next `[!mark` on the line (or end of line), trimmed. If empty, the fragment still gets the highlight style but no callout is rendered. A comment therefore can't contain the literal `[!mark`: it splits there instead.
- The comment marker itself (`// [!mark...]`) is stripped entirely from what the audience sees; only `<comment>` (if any) shows up, inside the callout box. Everything from the comment token onward is stripped, however many markers the line carries.
- A line's comment can carry **several markers**, processed left to right and sharing the range-pairing stack — so nested ranges can end (or start) on the same line, which a single marker per line couldn't express.
- Two substring markers on one line whose character ranges *overlap* aren't supported: the second is skipped with a console warning. Disjoint ranges on one line both render.
...
| Step range | `{N-M}`, `{N-}` or `{-M}` in place of `{<step>}`, e.g. `[!mark{2-4}]`, `[!mark{-0}]` | Visible at clicks N through M inclusive, then hidden again from M+1 (`{N-}` is `{N}`; `{-M}` is visible from click 0, `{-0}` only before the first click). A walk-through is `{-0}`, `{1-1}`, `{2}`. The click after the end counts toward the slide's clicks. Malformed ranges (`{0-2}`, `{3-2}`, `{-}`) make the marker unrecognized. |
...
| Several per line | `# [!mark:end] [!mark:end]`, `# [!mark(4-10)] note [!mark:end]` | Writes more than one marker in a line's trailing comment. Ends close innermost-first, so `[!mark:end] [!mark:end]` closes the inner range then the outer one; a line can also end one range and start another. |
...
- On `arrow` slides, `stackCallouts` then reorders each side's auto-placed boxes (per code block, or per slide callout's obstacle) by their anchors, top to bottom beside a block and left to right above or below it. Each box sits level with its anchor or just past the previous one, and the stack is pulled back inside the slide. Pinned boxes (`@x,y`, `box:`) stay put, and callouts whose step ranges never overlap don't constrain each other. So straight connectors don't cross in the common case (anchors far apart horizontally can still cross). `elbow` slides keep the greedy result exactly.
- In editor mode (Layout tab), drag a callout to override its position; the dragged position is written back into that callout's own marker as `@x,y` (e.g. `[!mark@120,40]`) via the `/api/save-code-highlight-position` endpoint, so it persists across reloads and survives further edits to the code above it. On a line carrying several markers only the dragged one is rewritten, and a line repeated on another slide is disambiguated by the slide's own line range.
...
- **Click steps** (`{<step>}`):
  - Placement runs over *all* callouts, stepped or not, in one pass, and hidden steps only get `visibility: hidden`, so revealing or hiding a step never moves a callout that's visible. Each placed box remembers its step range, and a callout only avoids the boxes that can be visible at a same click (`PlacedRect.range`, `rangesOverlap`): walk-through callouts sit beside their own highlight instead of shelf-stacking as if all were visible together. In editor mode, where everything is shown, such callouts may overlap.
  - The steps count toward the slide's clicks: `setup/transformers.ts` emits one zero-size `v-click` placeholder per distinct click at which the block's highlights change (each start, and the click after a range's end), because Slidev ignores click registrations made after mount. `layouts/default.vue` then checks each highlight's range (`data-highlight-click`, in its written form) against the slide's current click (`$clicks`).
  - In editor mode every callout is visible, and dragging a stepped callout keeps its `{<step>}` or range as written (the `@x,y` goes after it).
  - `slidev export --with-clicks` keeps each step as its own page.
  - Slidev's native `{1|3}` fence line ranges share the same click counter; how the two interleave is up to the author's numbering. They keep working on fences the theme wraps itself (because of markers or an inline `// [!source]` link): `wrapCodeBlock` parses the info string with `composables/fenceInfo.ts` (Slidev's own wrapper pattern) and forwards `:ranges` and the `{options}` binding, as Slidev's wrapper does.
...
| Click step | append `{<step>}` (or a step range `{N-M}`/`{N-}`/`{-M}`) after the anchor target (including any substring range, `+N` offset, `..` range, or `#N`/`#*`) and before any `@x,y`, e.g. `[!mark:3{2}]`, `[!mark:"text"#2{3-4}]` | Same behavior as the inline suffix. With `#*`, every match shares the step. |
...
`create-codeurjc-slidev --from-odp <file.odp> [--code <dir>] [--code-repo <github-url>]` scaffolds a project from a LibreOffice Impress deck (see "Multi-deck projects" below for several decks in one project), converting it on a best-effort basis. The importer is TypeScript in `packages/create-codeurjc-slidev/src/odp/`, bundled by `build.mjs` (esbuild, run on `prepack`) into `dist/odp-import.mjs`. The bundle inlines the theme composables it reuses; `index.mjs` only `import()`s it for `--from-odp`. Pipeline, one pure module per stage:

- `zip.ts`/`xml.ts`/`styles.ts`/`parse.ts` — ODF archive → plain `OdpDeck` model (`model.ts`): shapes with geometry, paragraphs with list depth/list-header flags, runs with effective bold/italic/monospace resolved through style chains, images, tables, connector endpoints and arrowhead markers, master body regions, hidden flags.
- `shapeGeometry.ts` — `draw:transform` (rotate/translate/skew/scale, applied in the order written; ODF's `rotate` turns counter-clockwise on screen) and geometry mirroring folded into one placement matrix per shape. From it come the rotated bounding box (`rect`, what LibreOffice's export reports), the endpoints of line-shaped custom shapes (`mso-spt32`, ...) and `arrowEnds` (tail/tip from line markers or a block arrow's type after rotation). Before this, a shape placed only by a transform had no `rect` and silently vanished from every stage.
...
- `diagrams.ts` — diagrams drawn with shapes, in three tiers.
...
- `office.ts`/`svg.ts`/`svgCrop.ts`/`comparison.ts` — LibreOffice ≥ 7.4 detection (up front, unless `office: false`) and one shared SVG export (isolated profile).
  ...
  - **Diagram crops.** `svgCrop.ts` crops each SVG candidate: every exported shape is a group with a `BoundingBox` (1/100 mm). Members are matched by centre and size with a size-relative slack, because LibreOffice draws picture borders, arrowheads and rotated outlines differently from the ODP frame, and overflowing text grows a box vertically. Pairs are assigned best first. The matched groups alone are written under `public/images/diagram-<page>.svg` and positioned with `geometry.images` after the slide's other images. An unmatched member makes the diagram one loss.
- `convert.ts` orchestrates; `index.ts` writes the project and formats the console report. Reports carry `losses` and `info` (conversions that lost nothing but deserve a look: mermaid, embedded SVG), printed in separate sections. Only losses feed the comparison deck.
- `importReport.ts` — the markdown import report, written last by `importOdpProject` to `import-reports/import-report-<local YYYY-MM-DDTHH-MM-SS>[-N].md` (a suffix on a same-second clash, so a report is never overwritten).
  - **Contents:** context (ODP, date, version, code folder, source-link base, LibreOffice), summary and notices, one row per code block with its match outcome (`ConvertResult.codeBlocks`: imported / close / none / command / no-folder), and notes and losses per slide. Each losses row gives its information slide in `comparison.md` (`ConvertResult.comparisonSlides`, counted in the same walk that builds the deck).
  - **Purity:** rendering is pure, and the clock and version are injected.
  - **Re-imports:** `project-dir.mjs` (beside `index.mjs`) keeps a top-level `import-reports/` when a re-import empties the target directory, and doesn't count it when deciding whether the directory is empty. The template's `.gitignore` ignores it.

**Multi-deck projects.** A project can hold several decks (`<slug>.md`) sharing one dependency tree, `code/` and `public/`; nothing per-deck is generated in `package.json`, because `pnpm dev tema1.md` (pnpm forwards extra args, Slidev takes the entry as a positional) already opens any deck. The CLI (`index.mjs`) builds one batch of `{ slug, odpPath? }` entries before writing anything: `--from-odp-dir <dir>` (every `.odp` in it, slug from the file name), repeatable `--from-odp` with `--deck` paired by position, or a bare `--deck <slug>` (empty deck). A lone entry without `--deck` is `slides`; a duplicate slug in the batch aborts before any write.
- **Recognized project.** A target directory whose `package.json` depends on `codeurjc-slidev-theme` (`isRecognizedProject` in `project-dir.mjs`) is added to, never wiped: only the batch's own decks are placed. Any other non-empty directory keeps the old "remove existing files (import reports are kept)?" prompt (`emptyDir`).
- **Existing decks.** A slug whose `<slug>.md` exists asks before overwriting just that deck (its file, code/image subfolders, comparison file; never `import-reports/` or root-level shared files); declining skips it. `--yes`/`--force` and `--skip-existing` answer for every deck (mutually exclusive); without a TTY it skips, since `prompts` would otherwise end the process silently. A summary line closes a batch run.
- **Namespacing.** Files land flat (`slides.md`, `code/`, `public/images/`, `comparison.md`, `public/odp-originals/`) only when the slug is `slides`, the project wasn't recognized before this run, and the batch has one entry; otherwise under `<slug>` (`code/<slug>/`, `public/images/<slug>/`, `public/odp-originals/<slug>/`, `<slug>-comparison.md`). `index.mjs` decides once and passes `deckFile`/`codeBase`/`imagesBase`/`originalsBase`/`comparisonFile` to `importOdpProject` → `convertOdp`, which bake them into the emitted `<<< @/…` paths, image `src`s and the comparison deck's `src: ./<deck>.md#N`; the import report names the deck file. A person can still put shared code/images at the root of `code/`/`public/images/`, since the code root and image `src`s are one flat tree.
- **Scripts.** `export` (`--with-clicks`) and `dev:compare` are only written for a project created around a single deck.
- **Editor recommendations.** Every run ensures `.vscode/extensions.json` (`editor-config.mjs`) recommends `codeurjc.vscode-codeurjc-slidev` and `antfu.slidev`, in fresh and recognized projects alike, adding only what is missing. A file that isn't valid JSON, or has comments and would need changes, is left as it is with a note. The comparison deck's first slide carries `comparisonDeck: true` so the VS Code extension can tell it from a presentable deck.
...
- `src/documentScan.ts` — shared raw-text scanning: fenced code blocks (with their info string), `<<<` import + directive-line blocks, and `splitSlides`, a port of `@slidev/parser`'s slide split (per-slide frontmatter, fences and HTML comments), which slide numbers come from.
- `src/markerDecorations.ts` — pure translation from a fence's marker/highlight parsing (via `parseCodeHighlights`) to document-absolute line/character positions, for dimming marker text and boxing highlighted spans in the open buffer, plus the click-step badges.
- `src/clickModel.ts` — counts each slide's clicks from its markdown the way Slidev does: every click source registers when it mounts (children before parents), a relative `at` starts after the relative clicks registered so far, an absolute one shifts nothing, and the total is the highest click reached unless frontmatter `clicks:` sets it. Recognises theme marker/anchor steps and step ranges (registered at their start and the click after their end; anchors resolved against the imported file, so an unmatched one registers nothing), slide callouts' `step:` values and ranges (read from the raw frontmatter YAML on `default`-layout slides), fence `{ranges}` with `{at}`/`{startLine}` options, `v-click`/`v-click-hide`/`v-after` attributes and components, `<v-clicks>` over a markdown list (`depth`, `every`, `at`) and `<v-click-gap>`. Anything it can't count (custom components — PascalCase, or kebab-case ones found under the project's `components/` — bound values, `<v-switch>`, `v-mark`/`v-motion`, magic-move, MDC `{v-click}` attributes, KaTeX ranges, a `<v-click>` wrapping another source) makes the slide *uncountable*: the total becomes null, absolute steps stay exact, and relative registrations after it lose their numbers.
  - **Ground truth.** `src/__tests__/groundTruthDeck.ts` is one slide per source form plus what Slidev 52.19.1 really does on each. `clickModel.groundTruth.spec.ts` checks the model against it, and `tests/click-model-ground-truth.spec.ts` (isolated project) checks real Slidev against it: each slide's `window.__slidev__.nav.clicksTotal` and the click at which each fence range segment highlights. A Slidev upgrade that changes click counting fails the e2e test.
- `src/stepBadges.ts` — per-line `▸N[,M] [of T]` badges (`formatStepBadge`) and hover text from the model.
- `src/importAnalysis.ts` — hovers, diagnostics, and CodeLens data (`codeLensActions`, one per `<<<` import block: the resolved file/line-range to open, plus the resolved source-link URL if any) for `<<<` imports and their anchor/`[!source]` directive lines, reusing `resolveSnippetSelector` and `parseExternalHighlightAnchors`'s `onWarn`/`onError` hooks (each anchor line is resolved individually so a diagnostic can be attached to its exact line). Also diagnoses two conditions the theme itself only ever logs to the console: an import resolving outside the configured code root (`ResolvedImport.escapesCodeRoot`, computed by the injected `ResolveImport`'s real fs-backed implementation via `referenceIndex/scanner.ts`'s `resolveImportTarget`) and a source link with no resolvable git branch (via the injected `ResolveSourceLink`, only for the "found a repo + GitHub remote but no branch" case -- no repo or a non-GitHub remote are the theme's own intentional silent-no-link states, not diagnosed). The hover for an auto-mode `[!source ...]` shows the real resolved URL, not a placeholder -- it's the same `ResolveSourceLink` call the CodeLens data reuses, computed once per import block. Reading the imported file and resolving its source link are both injected, so this stays testable without touching disk or a real git repo.
- `src/sourceLinkDiagnostics.ts` — wraps the theme's `resolveRepoLinkInfoCached`/`buildGithubSourceLink` (from `useSourceLink.ts`) into one `ResolveSourceLink` call answering both whether a source link resolves (`'ok' | 'no-repo' | 'no-remote' | 'no-branch'`) and, when it does, the actual URL -- one function rather than two overlapping ones re-deriving the same git state, reusing its injectable `GitRunner` for the same test-without-a-real-repo pattern the theme package itself uses.
- `src/referenceIndex/` — the reverse-reference feature: `indexBuilder.ts` builds, per target file, a *recipe* of which slide/anchor-line/selector combinations reference it (no file reads, no absolute line numbers yet); `codeLens.ts` resolves those recipes against a target file's live editor text on demand, so an edit to the target file alone is reflected immediately without any index invalidation — only an edit to the markdown file itself requires rebuilding that file's contribution (`updateReferenceIndexForFile`). Lens titles add a stepped anchor's click (`Slide 4 ▸3 of 3`), counting the referencing slide with the click model. `scanner.ts` is the only fs-touching part (workspace enumeration + `@/...` path resolution against the code-root convention, plus `listCodeRootDirectory` for path completion below and `listProjectComponentNames` for the click model).
- `src/pathCompletion.ts` — pure logic for `<<< @/code/...` path completion: `computeImportPathContext` determines whether the cursor sits inside a snippet-import's file-path token and which code-root-relative directory to list, `filterPathEntries` narrows by what's typed so far. Directory reading itself is `scanner.ts`'s `listCodeRootDirectory`, kept out of this module entirely.
- `src/selectorFromSelection.ts` — the decision tree behind the "Copy Selector for Selection" command: prefers a content-anchor range (`"first line".."last line"`) over a line range when both boundary lines are non-blank and occur exactly once in the file — this uniqueness check is a genuine safety measure, not extra caution, since `resolveSnippetSelector`'s content-anchor resolution always picks the *first* substring match with no ambiguity detection of its own (unlike highlight anchors' `#N`/`#*` occurrence selectors). Has no theme-side consumer (the theme only ever *resolves* an already-written selector, never *chooses* one), unlike `serializeSnippetSelector` (`useSnippetImport.ts`), the write-back splicing primitive this command's paste counterpart calls into the theme package for.
- `src/geometry/` — per-slide `geometry` support, pure unless noted:
  - `candidates.ts` builds, from a slide's markdown, the same DOM-free summary `layouts/default.vue` builds from the DOM (`ElementCandidate[]` grouped by kind — every code block, then mermaid, then tables, as the layout queries them —, image srcs, ids), so diagnostics are just `parseSlideGeometry` + `resolveGeometryElements`' own warnings (`diagnostics.ts`), mapped onto the field they name via `yaml` CST ranges (`frontmatterRanges.ts`). Slides with a `src:` include (`slideIncludes.ts`) keep grammar diagnostics but skip resolution ones: their content isn't in this file.
  - `completions.ts` offers `code:`/`id:`/`src:`/`image:`/`fit:` values from the slide's own content (images in `imageRefFor` form); `quickFixes.ts` adds a quoted `{id: '…'}` to a fence for an unmatched `id:` entry, or gives an unkeyed fence an id and an entry — the writes the Layout tab deliberately never makes.
  - `applyDrag.ts` applies a drag from the preview to the buffer by key: found on the dragged slide → apply (a stale `from` still applies); found on another slide → slide identity drifted, nothing applied; found nowhere → gone. `liveLink.ts` has deck matching, SSE parsing and the entry under the cursor; `devServer.ts` (impure) is the HTTP/SSE client.
- `src/geometryLiveLink.ts` — thin vscode adapter for "Inspect Geometry in Preview": attaches to `codeurjcSlidev.devServer.url`, claims control and turns inspection on, highlights the entry under the cursor, and applies drags as one undoable `WorkspaceEdit` followed by one save (`codeurjcSlidev.geometry.saveOnDrag`, default on: whole-file, so pending edits and format-on-save come along). Nothing slide-addressed is sent or applied until the page's announced deck is the open document. The official Slidev extension (`antfu.slidev`) is an `extensionPack` companion, not a dependency: it provides the preview, and a browser tab works too.
- `src/deckDiscovery.ts`, `src/includePlan.ts`, `src/deckTip.ts` (pure) and `src/deckDiscoveryAdapter.ts` (thin vscode adapter) — deck discovery for the official Slidev extension, which only registers `slidev.include` matches (default `**/slides.md`), so multi-deck projects (`<slug>.md`) register nothing.
  - **Decks.** A markdown file declaring the theme, except comparison decks (`comparisonDeck: true` in the headmatter, else named `comparison.md`/`*-comparison.md`).
  - **Registration.** Decks that `workspace.findFiles` over the current `slidev.include` doesn't reach are offered for registration (Register / Not now, a declined set isn't re-offered). Accepting writes `slidev.include` at **Workspace** scope (a window-scoped setting: VS Code ignores it in a folder's settings in multi-root), missing decks first as literal paths relative to their workspace folder (VS Code glob has no backslash escape, so each of `*?[]{}` is wrapped in a character class, `]` included: verified against `findFiles`, where a lone `]` breaks the match), then the existing entries. Nothing is written without the Slidev extension's setting being registered.
  - **Default deck.** The Slidev extension breaks ties between equally shallow entries by `include` order, so the deck last edited (tracked in `workspaceState`), else alphabetical, goes first. A choice it already remembers wins; its active entry can't be read.
  - **Tip.** The first time a deck of a project with two or more decks becomes the active editor, once per workspace, an information message offers "Choose deck…" (`slidev.choose-entry`). `codeurjcSlidev.deckDiscovery.enabled` silences both.
- `src/extension.ts` — thin adapter wiring the above to real `vscode` decorations/Hover/Diagnostic/CodeLens/CompletionItem providers and the two selector commands; deliberately not unit tested itself (see Tests below). Badges are one `after`-text decoration type; the `codeurjcSlidev.stepBadges.showTotal` setting (the extension's only one) repaints editors and lenses when changed. `activate()` returns `{ stepBadgesFor(uri) }`, the badges last painted per document, purely so the extension-host smoke tests can check decorations (VS Code can't read them back).
...
pnpm test:e2e:vscode        # the geometry live link, end to end: real VS Code + real Slidev + real Chromium (needs a display)
pnpm test:e2e:vscode        # the geometry live link, end to end: real VS Code + real Slidev + real Chromium (needs a display)
```

## CI

`.github/workflows/test.yml` runs on every push/PR to `main`, as independent parallel jobs: `lint` (`pnpm lint`), `typecheck` (`pnpm typecheck`), `unit-test` (`pnpm test`), `e2e` (`pnpm test:e2e`, after `npx playwright install --with-deps chromium`), and `build` (`pnpm build`, this repo's own `slides.md` as a regression smoke test). `.github/workflows/extension-test.yml` runs the slower, Electron-based `test:extension` smoke suite via `xvfb-run` on push to `main` only, path-filtered to `packages/vscode-codeurjc-slidev/**` and `packages/codeurjc-slidev-theme/**` (the live-link e2e, `test:e2e:vscode`, also runs there) — deliberately not a PR-blocking check, since it downloads a full VS Code build.
