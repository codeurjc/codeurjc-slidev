...
- **Deck discovery** — the official Slidev extension only knows `slides.md` files by default, so a project with several decks (`tema1.md`, `tema2.md`, …) shows nothing in its sidebar. This extension finds every deck (a markdown file with `theme: codeurjc-slidev-theme`, comparison decks excluded) and offers to add the ones it doesn't know to the `slidev.include` workspace setting, the deck you last edited first so it becomes the default. The first time you open a deck of a project with several, a tip explains how to switch the active deck (**Choose deck…**, or the Projects view in the Slidev sidebar). `codeurjcSlidev.deckDiscovery.enabled` turns both off.
